...
describe('current-task-baseline OMX parity adapter', () => {
  ...
  it('updates branch lifecycle metadata when PR info is observed', async () => {
    await withTemp(async (repo) => {
      ...
      const raw = JSON.parse(await readFile(join(repo, '.omc', 'state', 'current-task-baseline.json'), 'utf-8')) as {
        tasks: Array<{ branch_name: string; issue_number?: number; pr_number?: number; status?: string }>;
      };
