...
function legacyOmxBaselinePath(repoRoot: string): string {
...
export function readCurrentTaskBaseline(repoRoot: string): CurrentTaskBaselineFile {
  ...
  if (existsSync(path)) return readBaselineFile(path);

  const legacyPath = legacyOmxBaselinePath(repoRoot);
  if (existsSync(legacyPath)) return readBaselineFile(legacyPath);
